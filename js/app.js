// 全局变量
let selectedAPIs = JSON.parse(localStorage.getItem('selectedAPIs') || '["modu", "heimuer", "mozhuazy", "bfzy", "rycj", "wolong"]'); // 默认选中资源
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        // 默认选中资源
        selectedAPIs = ["modu", "heimuer", "mozhuazy", "bfzy", "rycj", "wolong"];
...
async function search() {
    ...
    try {
        ...
        const safeResults = allResults.map(item => {
            ...
            const safeCoverUrlAttr = hasCover ? item.vod_pic.replace(/"/g, '&quot;') : '';
                ...
                <div class="card-hover bg-[#111] rounded-lg overflow-hidden cursor-pointer transition-all hover:scale-[1.02] h-full shadow-sm hover:shadow-md" 
                    ...
                    <div class="flex h-full">
                        ...
                        <div class="relative flex-shrink-0 search-card-img-container">
                            <img src="${safeCoverUrlAttr}" alt="${safeName}" 
                                 class="h-full w-full object-cover transition-transform hover:scale-110" 
                                 data-original-src="${safeCoverUrlAttr}"
                                 data-fallback-src="https://via.placeholder.com/300x450?text=无封面"
                                 onerror="handleImageProxyError(this)" 
